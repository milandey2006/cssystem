...
// When `preferredSizeTB` is a positive number, the caller has pinned a specific
// drive size, so we just work out how many of that size are needed instead of
// auto-choosing the fewest/smallest drives.
export function recommendDriveConfig(requiredTB, preferredSizeTB) {
  if (preferredSizeTB && preferredSizeTB > 0) {
    const count = Math.max(1, Math.ceil(Math.max(requiredTB, 0) / preferredSizeTB));
    return { count, sizeTB: preferredSizeTB, totalTB: count * preferredSizeTB };
  }
