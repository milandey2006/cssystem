...
export default function ProductsPage() {
      ...
      try {
        const productsData = await client.fetch(`
          *[_type == "product"]{
            ...
            reviewCount,
            keyFeatures,
            brand, // Fetches the single string field 'brand'
  ...
  useEffect(() => {
    ...
    // Brand filter (Robust logic: trims whitespace and matches selected brands)
    if (selectedBrands.length > 0) {
      filtered = filtered.filter(product => {
        ...
        return selectedBrands.some(brand => 
          // Use trim() to eliminate hidden spaces, ensuring a strict match
          productBrand.toLowerCase().trim() === brand.toLowerCase().trim()
        )
      })
    }
    ...
    setCurrentPage(1)
  }, [searchTerm, selectedCategories, selectedPriceRanges, selectedFeatures, selectedBrands, products])
  ...
  const hasActiveFilters = selectedCategories.length > 0 || selectedPriceRanges.length > 0 || 
                         selectedFeatures.length > 0 || selectedBrands.length > 0 || searchTerm 
  ...
  // ... (Pagination Component content remains unchanged)
  const Pagination = () => {
      ...
      <div className="container mx-auto py-6 lg:py-12 px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:hidden">
            ...
            >
              <Filter size={16} />
              Filters {hasActiveFilters && `(${selectedCategories.length + selectedBrands.length + selectedFeatures.length + selectedPriceRanges.length})`}
            </Button>
          </div>
          ...
          <div className="flex-1">
            ...
            <div className="relative mb-6">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4 fixed" />
              <Input
